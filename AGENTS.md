...
- Outside tests, never discard the `Result` from `run(...)`: either inspect a non-ok result and show the appropriate UI error, or use `run.orThrow(...)` inside error handling that shows a toast (the inline-edit controls already toast rejected saves). Ignoring `await run(...)` is allowed only in tests.
