...
- User-facing copy never names Linky. The shared identity, wallet and relay list are described as belonging to "every app signed in with your recovery phrase"; Linky is named only in code comments and this documentation.
...
- `src/core/background-jobs` contains the background job framework (`BackgroundJobContext`, keyed task queue) and the sync jobs under `jobs/`. Jobs receive all effects — including `lockManager` — through their context; never use ambient globals such as `navigator.locks`. `jobs/nostr-cashu-inbox-job.ts` settles cashu payments paid as ecash: it reads the account's Nostr inbox (`src/core/linky/nostr-inbox.ts`) through the `NostrInboxDep` in its context (`src/core/linky/nostr-inbox-source.ts`; `null` in the CLI), receives what arrives through `CashuWallet.receiveToken` and claims the payment by request id or by mint and amount. Inject `subscribeGiftWraps` to test it without relays.
