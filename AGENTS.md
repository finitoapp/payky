...
- For a React call site invoking a Task whose error type is `never` (its only realistic failure is an unexpected infra error, not a domain `Result`), don't build bespoke pending/error UI: run it through `useRunToast()` (`run.ok(task)` inside the callback) and let the surrounding UI (button, dialog) act optimistically — close/navigate immediately rather than waiting on the mutation. See `saveFiatCurrency` in `_terminal.settings.fiat.tsx` and the catalog-item delete confirmation in `item-form-page.tsx` (via `useConfirmedRun()`) for this pattern.
