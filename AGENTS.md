...
- `src/core/migrations` contains the data migrations the app runs at start; `migrations.ts` holds the runner and the ordered `appMigrations` registry, and its `AppMigration` doc comment says what `hasWork` and `run` must guarantee. A migration lives in its own file (`account-derived-id-migration.ts`) that exports one `AppMigration`, with its test beside it. Everything it needs only for itself — queries, row types, derivations, the writes — stays inline in that file, even when that makes it long: it is single-use code that gets deleted with the migration, so don't extract its pieces into helpers or into the domain modules. Reuse a module's existing exports freely; the one piece worth a local function is the work plan `hasWork` and `run` both read, so the two cannot disagree.
...
- `src/core/native` contains Capacitor/WebView runtime detection and platform plumbing. `switchio.ts` wraps the SwitchioPay card terminal, whose ECR protocol is Android-intent-only — the native half is a local Capacitor plugin at `android/app/src/main/java/me/payky/SwitchioPlugin.java`, registered in `MainActivity.java`. There is no PWA equivalent: an `intent://` launch returns its result to the browser, not to the page, so card payments are hidden outside `Capacitor.isNativePlatform()`. A result that arrives after Android killed the app mid-payment comes back as a Capacitor `appRestoredResult` event, which `src/components/app/switchio-restored-result.tsx` settles.
...
- In React, obtain runs through `useAppRun()` from `src/hooks/use-app-run.ts`: `const appRun = useAppRun()` then `await using run = appRun()` inside handlers. Do not call `createRun` directly in components; the only sanctioned exception is `app-background-jobs.tsx`, which additionally needs `lockManager` and `onError`. For a handler whose only failure UI is a generic toast, prefer `useRunToast()` (`src/hooks/use-run-toast.ts`) — or `useConfirmedRun()` when a confirm dialog gates it first — over calling `useAppRun()` directly; both own the `await using run = appRun()` scope for you.
...
- Outside tests, never discard the `Result` from `run(...)`: either inspect a non-ok result and show the appropriate UI error, or run the Task through `useRunToast()`'s `action` callback, returning a `TranslationKey` for a specific failure message, or nothing (falling off the end) on success — `run.ok`/`run.orThrow` inside that callback throw on failure and `useRunToast` converts the throw into its generic fallback toast. Do not hand-roll `try { await using run = appRun(); await run.orThrow(...) } catch { toast.error(...) }` at a call site; that's exactly what `useRunToast` replaces (inline-edit controls get the same `TranslationKey | undefined` protocol from `useInlineSave` instead). Ignoring `await run(...)` is allowed only in tests.
...
- For a React call site invoking a Task whose error type is `never` (its only realistic failure is an unexpected infra error, not a domain `Result`), don't build bespoke pending/error UI: run it through `useRunToast()` (`run.ok(task)` inside the callback) and let the surrounding UI (button, dialog) act optimistically — close/navigate immediately rather than waiting on the mutation. See `move` (the payment-method reorder) in `payment-accounts-settings-page.tsx` and the catalog-item delete confirmation in `item-form-page.tsx` (via `useConfirmedRun()`) for this pattern.
