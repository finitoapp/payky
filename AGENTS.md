...
- `bun run sync:linky [path-to-linky]` re-vendors the Linky packages (`@linky/linkshu`, `@linky/linksync`, `@linky/profile-defaults`) from a Linky checkout (default `../linky`) into `packages/`; see the vendored-package rule under "Project Structure".
